...
public class AuthService {
	 ...
	 * active. If user is already verified, save the unnecessary database calls.
	 */
	...
	/**
	 * Attempt to regenerate a new email verification token given a valid previous
	 * expired token. If the previous token is valid, increase its expiry else
	 * update the token value and add a new expiration.
	 */
	...
	public Optional<RefreshToken> createAndPersistRefreshTokenForDevice(Authentication authentication,
		...
		userDeviceService.findByUserId(currentUser.getId()).map(UserDevice::getRefreshToken).map(RefreshToken::getId)
	...
	public Optional<String> refreshJwtToken(TokenRefreshRequest tokenRefreshRequest) {
		...
		return Optional.of(refreshTokenService.findByToken(requestRefreshToken).map(refreshToken -> {
			...
			return refreshToken;
		}).map(RefreshToken::getUserDevice).map(UserDevice::getUser).map(CustomUserDetails::new)
	...
	public Optional<User> resetPassword(PasswordResetRequest passwordResetRequest) {
		...
		return Optional.of(passwordResetToken).map(PasswordResetToken::getUser).map(user -> {
